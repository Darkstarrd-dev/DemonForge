...
export async function gptImageRoutes(app: FastifyInstance) {
  ...
  app.post('/api/image/gpt-generate', async (req, reply) => {
    const { baseURL, apiKey, model, prompt, size, quality, background, moderation, imageInputs } =
    ...
    generateImageGpt(
      { baseURL, apiKey, model, prompt, size, quality, background, moderation, imageInputs },
