...
### 阶段 A：编译产物优化（✅ 已完成，2026-06-22）

**实施内容**：
1. ✅ 懒加载大模块
   - image-helper, node-test, role-chat, settings
   - 使用 `lazy()` + `<Suspense fallback={<Spin />}>`
...
3. ✅ 编译验证
   - 主包：1.8 MB → **180 KB**（gzip 58 KB）
   - 编译时间：659ms
   - 警告：vendor-3d/vendor-2d 超过 1000 KB（预期，已独立 chunk）
...
### 阶段 B：settings 页内部重组（✅ 已完成，2026-06-22）

**实施内容**：
1. ✅ 提取 4 个 Tab 渲染内容为独立函数组件
   - `NodesTabContent` (~200 行)：Provider 节点池 + 模块映射 + M1 提示词 + 测试文本
   - `AdvancedTabContent` (~60 行)：章节检测模式池 + 资产目录
   - `GeneralTabContent` (~50 行)：主题 + 菜单栏 + 4K 缩放
   - `BackupTabContent` (~120 行)：导入导出 + 完整备份
2. ✅ 主渲染函数简化
   - Tabs items 改为组件调用 + props 传递
   - 删除旧的内联 JSX（~520 行）
3. ✅ 编译验证
   - 前端编译通过（仅未使用导入警告，不影响功能）
   - 文件行数：1820 → 1799 行

**收益**：
- 逻辑分块清晰，各 Tab 内容独立
- 主函数可读性提升（Tabs 容器职责明确）
- 保持状态管理不变（降低重构风险）
...
### 阶段 C-E：评估与结论（✅ 已完成，2026-06-22）

**Step3Clean.tsx (1217 行)**：
- **评估结果**：核心逻辑为清理调度器控制（worker 循环、节点熔断、重试队列、实时窗口更新）
- **结论**：逻辑高度内聚，闭包访问调度器 handle，强行拆分会破坏内聚性且增加复杂度
- **决策**：保持现有结构

**image-helper/index.tsx (702 行主文件)**：
- **评估结果**：已拆分 LayerEditor.tsx (467 行) + GlobalCropPanel.tsx (430 行)
- **主文件内容**：画布控制 + 帧管理 + 导出（魔棒、裁剪、网格等工具函数）
- **结论**：主文件行数已满足可维护性目标
- **决策**：无需进一步拆分

**llm.ts (747 行)**：
- **评估结果**：单职责 LLM 清理调度器，函数式设计
  - `streamSingleChapter` (97 行)：单章 SSE 流式请求
  - `streamBatch` (165 行)：多章合并请求
  - `startCleanQueue` (365 行)：中央调度器（worker 循环 + 节点管理）
- **结论**：模块内聚，函数边界清晰，拆分收益低
- **决策**：保持现有结构

**总结**：
- 阶段 A 解决了核心问题（编译产物体积）
- 阶段 B 完成了最大文件（settings）的内部重组
