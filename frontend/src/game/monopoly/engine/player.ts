...
export function liquidate(player: Player, properties: Record<number, PropertyState>) {
...
export function handleBankrupt(state: GameState): GameState {
  ...
  const log = [...state.log]
  const current = players.find((p) => p.id === state.turn.currentPlayerId)
  ...
  if (alive.length <= 1) {
    ...
    return { ...state, players, properties, log, status: 'ended', winnerId }
  }
  return { ...state, players, properties, log }
}

export function calcTotalAssets(player: Player, properties: Record<number, PropertyState>, tilesPrice: Record<number, number>): number {
