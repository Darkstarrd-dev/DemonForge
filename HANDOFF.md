...
**最后更新**：2026-06-30
**当前位置**：办公场所 A（本轮待推送）
**本轮主题**：**节点池模块化批次3实施完成：5.4 状态 slice 解耦（NodePoolState 独立 store + interop 薄封装）**
...
## 🆕 节点池模块化方案 · 批次3实施完成（2026-06-30，待推送；批次1+2+3合并待提交）
...
- **`store/bootstrap.ts`**：
  - 删除本地 `OldProviderNode` 接口与迁移逻辑
  - 启动 hydrate 改调 `hydrateNodePoolState` → `nodePoolStore.setState`
- **`pages/settings/index.tsx`**：
  - `confirmImportSettings` / `handleImportNodePool` 中 providers/providerNodes/moduleMapping 增量导入改走 `nodePoolStore.setState`
  - `reorderProviders` / `reorderNodes` 改走 `nodePoolStore.setState`
  - 防止 AppState 与 nodePoolStore 状态漂移
- **新增单测 `frontend/src/packages/node-pool/store.test.ts`**：15 个用例覆盖初始态、供应商/节点 CRUD、级联清理 moduleMapping、`consumeProviderUsage`（含跨日重置）
...
### 回滚策略
- 本次为纯重构 + 文件新增，无数据迁移；已备份 `server/src/data/settings.json.pre-batch3.bak`
- 若 hydrate 有 bug，可恢复备份或 `git revert`
...
## 🆕 节点池模块化方案 · 批次2实施完成（2026-06-30，待推送）

按 `docs/node_pool_modularization_plan.md` §11 批次2实施 5.3 调度策略抽出。

### 改动

- **新增 `frontend/src/packages/node-pool/runtime.ts`**：`NodeRuntime` + `NodeRuntimeMap` 运行时态类型
- **新增 `frontend/src/packages/node-pool/policy.ts`**：
  - `isNodeAvailableNow(cfg, state, opts)`：统一可用性判定（并发 / 间隔 / 外部次数限制）
  - `pickLeastLoadedNode(configs, states, opts)`：最久未用 → 最少连接选择策略
- **`packages/node-pool/types.ts` 追加 `SchedulableNode`**：调度器消费的运行时节点子集，`batchChars` 可选以兼容 BatchGenNode
- **`services/real/llm.ts`**：`CleanNode` 改为 `SchedulableNode & { batchChars: number }` 别名，消除与 `BatchGenNode` 的重复定义
- **`services/real/batch.ts`**：
  - `BatchGenNode` 改为 `SchedulableNode` 别名
  - 删除本地 `NodeRuntime` 与 `pickCandidate`
  - worker 循环改调 `pickLeastLoadedNode`
- **`services/real/cleanScheduler.ts`**：
  - 删除本地 `NodeRuntime`，改从 `runtime.ts` import `NodeRuntimeMap`
  - `workerLoopForNode` 可用性检查改调 `isNodeAvailableNow`
- **`pages/book-reader/ImmersiveReader.tsx`**：新增 `toSingleNodeClean(node)` 薄适配函数，替代 6 行手工构造；单章清理仍覆写 `maxConcurrency:1 / batchChars:999999 / intervalSec:0`
- **`services/types.ts`**：re-export `SchedulableNode` / `NodeRuntime` / `NodeRuntimeMap`，保持现有调用方兼容
- **新增单测 `frontend/src/packages/node-pool/policy.test.ts`**：13 个用例覆盖 `isNodeAvailableNow` 与 `pickLeastLoadedNode`
- **顺手修复 `serialize.test.ts`**：`moduleMapping: {}` 补类型断言，消除 tsc 报错

### 验证

| 命令 | 结果 |
|---|---|
| `npx tsc -b`（frontend） | 4 个预存错误，本轮无新增（Step3Clean.tsx ×3 / ModelMappingModal.tsx ×1） |
| `bun run lint`（frontend） | 2 个预存错误（Step3Clean.tsx parseSSE/ProviderNode 未用），本轮无新增 |
| `bun run test:core`（frontend） | **76/76 passed**（新增 13 个 policy 用例） |
| `npm test`（frontend 全量） | **451/451 passed** |
| `npx tsc --noEmit`（server） | 通过 |
| `npx tsc --noEmit`（electron） | 通过 |

### 回滚策略
- 本次为纯重构 + 文件新增，无数据迁移。
- 如出现异常，可 `git revert` 或单独恢复 `batch.ts`/`cleanScheduler.ts` 原实现。

### 下一步
- 批次3：5.4 状态 slice 解耦（`NodePoolState` 独立 store + interop 薄封装）
- 批次4：5.5a 后端独立路由（过渡）
- 批次5：5.6 UI 组件拆分
...
- [x] **节点池模块化·批次 1**（5.1 类型独立 + 5.2 纯函数层独立 + 5.7 导入/导出独立，2026-06-30）
- [x] **节点池模块化·批次 2**（5.3 调度策略抽出：`runtime.ts`/`policy.ts`/`SchedulableNode`，2026-06-30）
- [x] **节点池模块化·批次 3**（5.4 状态 slice 解耦：`NodePoolState` 独立 store + interop 薄封装，2026-06-30）
### ✅ 已完成模块
...
1. **📐 按 `docs/node_pool_modularization_plan.md` 批次 4 实施节点池模块化**：5.5a 后端独立路由（`NodePoolRepository` 接口 + `SettingsJsonRepo` + `/api/providers`/`/api/nodes`）
2. **🔧 端到端实测节点池**：新增供应商（多 API KEY / Round-Robin / Failover）→ 新增文本/图片节点 → 模块映射 → 批量测试 → 拖拽排序 → 导入导出 → 旧 settings.json 迁移
