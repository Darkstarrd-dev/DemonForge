...
**最后更新**：2026-06-28
**当前位置**：办公场所 A
**本轮主题**：**大富翁 M4 道具系统**——按 `docs/monopoly_full_plan.md` §7 M4 里程碑实施。13 种道具全量效果引擎 + 道具店/研究所双购买通道 + 持有上限 5 + 耐久系统 + 棋盘陷阱（地雷/路障/定时炸弹）+ 交通工具切换（摩托车/汽车影响骰子数）+ 武器攻击（飞弹/炸弹/核子飞弹范围摧毁）+ 工具效果（吸尘器吸收过路费/机器娃娃清障/工程车拆除/传送器/遥控骰子）+ `resolveTraps` 陷阱触发 + `tickTimedBombs` 定时炸弹倒计时。`item.ts` 全量实现（422 行）：`createItemDeck`/`handleBuyItem`/`handleUseItem`/`applyItemEffect`/`buildItemChoiceDecision`/`resolveItemChoice`。`turn.ts` 集成陷阱检测与吸尘器租金吸收。`engine.ts` 集成 `rollDice` 支持交通工具骰子数。`GamePanel` 新增商店 Segmented 三标签（卡片/道具店/研究所）+ 道具栏（含耐久显示）+ 道具使用目标选择 Modal。27 新增单测覆盖全部道具类型与耐久系统。build 全绿 + vitest 174 绿（+27）。
...
1. **大富翁端到端实测 M0→M4**：开新局 → 掷骰买地 → 存取款 → 买股票 → 点数购买卡片/道具 → 使用卡片效果 → 反制链窗口 → 研究研发道具 → 使用飞弹/地雷/交通工具 → 切换台湾地图。
2. **大富翁 M5 神明系统**：13 种神明 + 附身机制 + 变身（小财神→大财神）+ 送神符 + 死神不可送（按 `docs/monopoly_full_plan.md` §7，依赖 M0）。
