...
1. **会话开始先读 `HANDOFF.md`**：项目在两个办公场所间通过 git 手动同步，`HANDOFF.md`
   是恢复工作的唯一入口（进展 checklist、下一步、交接备注）。
...
6. **每阶段实施后提交**：执行大富翁模块实施计划（`docs/monopoly_full_plan.md`）时，每完成一个里程碑（M0→M12）并通过验收（tsc 0 + eslint 0 + vitest 全绿）后，进行一次 git 提交。提交信息遵循 `type(scope): 描述` 格式。

## 工具链

- **内容搜索**：优先使用 `rg`（ripgrep），路径 `C:\Users\Houpy\.cache\opencode\bin\rg.exe`
